...
DXL_MINIMUM_POSITION_VALUE  = 250;          % Dynamixel will rotate between this value
DXL_MAXIMUM_POSITION_VALUE  = 512;         % and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
DXL_MOVING_STATUS_THRESHOLD = 5; % Dynamixel moving status threshold
DXL_MOVING_SPEED            = 100;
DXL_TORQUE_LIMIT            = 523;
DXL_MIN_SPEED               = 0;
...
dxl_present_speed = [0 2047];


dxl_error = 0;                              % Dynamixel error
dxl_present_position = 0;                   % Present position
dxl_Present_speed = 1;                      % Present Speed
...
write2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_TORQUE_LIMIT, DXL_TORQUE_LIMIT);
    dxl_comm_result = getLastTxRxResult(port_num, PROTOCOL_VERSION);
    dxl_error = getLastRxPacketError(port_num, PROTOCOL_VERSION);
    if dxl_comm_result ~= COMM_SUCCESS
        fprintf('%s\n', getTxRxResult(PROTOCOL_VERSION, dxl_comm_result));
